...
type IngestPublisher interface {
	Publish(context.Context, *ingestpb.IngestEvent) error
}
...
type Server struct {
	...
	producer    IngestPublisher
}

func NewServer(coordinator coordinatorpb.VectorSearchClient, embed embedpb.EmbedServiceClient, producer IngestPublisher) *Server {
	return &Server{coordinator: coordinator, embed: embed, producer: producer}
}
